...
while not done:
    while turn == "player" and not done:#players turn
        print()
        print("Player:", player_total, "Computer:", comp_total)
        print("It's your turn!")
        roll = random.randint(1,6)
        print("You rolled a", roll)
        if roll == 1:#turn is switched to computers
            turn = "computer"
            player_temp_total = 0
            print("PIG! Too bad! Your total is currently:", player_total)
        else:
            player_temp_total += roll #player scores
            ...
            if choice == 'n':
                ...
                turn = "computer"
        if player_total > winning_score:#player wins
            print("You win! " + str(player_total) + " to " + str(comp_total))
            done = True

    while turn == "computer" and not done:#computers turn
        print()
        print("Player:", player_total, "Computer:", comp_total)
        print("It's the computer's turn!")
        roll = random.randint(1,6)
        print("The computer rolled a", roll)
        if roll == 1:#turn is switched to players
            turn = "player"
            comp_temp_total = 0
            ...
            if comp_temp_total > 6 or comp_total + comp_temp_total > winning_score:
                ...
                turn = "player"
        if comp_total > winning_score:#computer wins
